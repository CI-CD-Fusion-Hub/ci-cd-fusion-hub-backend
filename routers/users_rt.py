...
@auth_required
async def get_all_users(request: Request, user_service: UserService = Depends(create_user_service)):
    return await user_service.get_all_users()


@router.get("/users/{user_id}", tags=["users"])
@auth_required
async def get_user_by_id(request: Request, user_id: int, user_service: UserService = Depends(create_user_service)):
...
@auth_required
async def delete_user(request: Request, user_id: int, user_service: UserService = Depends(create_user_service)):
